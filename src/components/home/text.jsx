...
const Text = ({ sectionRef }) => {
  ...
  return (
    <>
      ...
      <div className={`absolute inset-0 h-full w-full transition-opacity duration-500  ${isLoaded ? 'opacity-100' : 'opacity-0'}`} ref={imageref}>
        <video
          ref={videoRef}
          ...
          fetchPriority="high"
          className={`absolute inset-0 h-full w-full object-cover  transition-opacity duration-500  ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
        ></video>
      </div>
