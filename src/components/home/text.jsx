...
const Text = ({ sectionRef }) => {
  ...
  return (
    <>
      ...
      <div className={`absolute  inset-0 transition-opacity duration-500    ${isLoaded ? 'opacity-0 z-0' : 'opacity-100 z-20'}`}>
      ...
      <div className={`absolute inset-0 h-full w-full `} ref={imageref}>
        <video
          ...
          fetchPriority="high"
          className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-100 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
        ></video>
      </div>
